...
class ResNetWrapper(nn.Module):
    def __init__(self, model):
        super().__init__()
        self.model = model
        self.predictor = model.fc  # alias for compatibility
        # Rename fc to predictor for compatibility
        self.model.predictor = self.model.fc
        del self.model.fc

    def forward(self, x, return_embedding=False):
        # Handle batch size 1 for BatchNorm during training
        bn_eval = x.size(0) == 1
        if bn_eval:
...
class ResNet(fp.Module):
    ...
    def generate_model(self, input_data_shape, target_class_num, vgg_name='VGG11', *args, **kwargs):
        # Choose ResNet-18 or ResNet-50 depending on your experiments
        if self.name == 'resnet18':
            model = models.resnet18(weights=models.ResNet18_Weights.IMAGENET1K_V1)  # pretrained on ImageNet-1K
        elif self.name == 'resnet50':
            model = models.resnet50(weights=models.ResNet50_Weights.IMAGENET1K_V1)
        else:
            raise ValueError(f"Unsupported model name: {self.name}")

        num_classes = target_class_num  # CIFAR-100
        model.fc = nn.Linear(model.fc.in_features, num_classes)

        self.model = ResNetWrapper(model).to(self.device)
...
class ResNet_Model(nn.Module):
    def __init__(
        ...
        self.fc = nn.Linear(64 * block.expansion, target_class_num)
...
def ResNet18(device, **kwargs: Any) -> ResNet:
    ...
    """
    module = ResNet(device)
    module.name = 'resnet18'
    return module


def ResNet50(device, **kwargs: Any) -> ResNet:
    r"""ResNet-50 model from
    `"Deep Residual Learning for Image Recognition" <https://arxiv.org/pdf/2102.06171v1>`.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    module = ResNet(device)
