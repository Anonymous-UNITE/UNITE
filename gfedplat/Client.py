...
class Client:
    ...
    def get_message(self, msg):
        ...
        if msg['command'] == 'train':
            ...
            epochs = msg['epochs']
            lr = msg['lr']
            local_upt_part = msg.get('local_upt_part', 'all')

            if self.sgd_step:
                self.train_SGD(epochs, lr, local_upt_part)
            else:
                self.train(epochs, lr, local_upt_part)
            return return_msg
    ...
    def train_SGD(self, epochs, lr, local_upt_part='all'):
    ...
    def train(self, epochs, lr, local_upt_part='all'):
        self.upload_module = copy.deepcopy(self.module)

        # --- DEBUG CHECK 1: Verify Freezing State ---
        trainable_params = sum(p.numel() for p in self.upload_module.model.parameters() if p.requires_grad)
        total_params = sum(p.numel() for p in self.upload_module.model.parameters())
        #print(f"Client {self.id} (Part: {local_upt_part}): Trainable Params={trainable_params}, Total Params={total_params}", flush=True)

        # --- NEW LOGIC: Filter parameters based on the server's intent ---
        if local_upt_part == 'head':
            # Parameters were already frozen by the server (body=False, head=True)
            pass
        elif local_upt_part == 'body':
            # Parameters were already frozen by the server (body=True, head=False)
            pass
        elif local_upt_part == 'all':
             # Unfreeze all just in case
             for param in self.upload_module.model.parameters():
        ...
        # --- DEBUG CHECK 2: Verify Optimizer Tracks Correctly ---
        opt_params = sum(p.numel() for group in optimizer.param_groups for p in group['params'])
        if opt_params != trainable_params:
             raise RuntimeError(f"Optimizer tracking mismatch! {opt_params} vs {trainable_params}")
        #print(f"Client {self.id} (Part: {local_upt_part}): Optimizer tracks {opt_params} parameters.", flush=True)
